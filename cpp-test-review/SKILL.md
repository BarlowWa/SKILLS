...
- **MOCK-003**：每个测试用例中 mock 对象数量应控制在合理范围（建议 ≤3）。超出时说明被测类职责过重，应重构而非堆砌 mock
...
- **COV-001**：每个测试用例在验证被测功能的正常路径之外，必须覆盖至少一个边界条件或异常/错误路径。禁止仅有 happy-path 的测试集
...
- **PERF-001**：单个测试用例平均执行时间应控制在 100ms 以内。超过 500ms 的测试应标记为慢速测试并确认必要性
- **PERF-002**：禁止在测试中使用 `sleep()` / `std::this_thread::sleep_for()` 等主动等待——用条件变量、fake clock 或轮询超时替代
...
- [ ] DEATH：Death Test —— DEATH-001：`tests/parser_test.cpp:56` death test 用于验证 `std::runtime_error`，应改用 `EXPECT_THROW`
- ...
...
- 性能基准测试（benchmark）不受 PERF-001/PERF-002 约束，使用独立的 `*_bench.cpp` 或 `bench/` 目录
